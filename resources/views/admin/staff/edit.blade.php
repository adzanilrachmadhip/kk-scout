                        ...
                        <form action="{{ route('staff.update', $staff->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div id="education-fields">
                                @foreach ($staff->education as $index => $education)
                                    @php
                                        $degree = old('education.' . $index . '.degree', $education->degree);
                                        $isOther = !in_array($degree, ['Doctor of Philosophy', 'Master Degree', 'Bachelor Degree']);
                                    @endphp
                                    <div class="row education-row" data-index="{{ $index }}">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <select class="form-control degree-select" name="education[{{ $index }}][degree]" onchange="toggleOtherDegree(this, {{ $index }})">
                                                    <option value="">Select Degree</option>
                                                    <option value="Doctor of Philosophy" {{ $degree == 'Doctor of Philosophy' ? 'selected' : '' }}>Doctor of Philosophy</option>
                                                    <option value="Master Degree" {{ $degree == 'Master Degree' ? 'selected' : '' }}>Master Degree</option>
                                                    <option value="Bachelor Degree" {{ $degree == 'Bachelor Degree' ? 'selected' : '' }}>Bachelor Degree</option>
                                                    <option value="Other" {{ $isOther && $degree ? 'selected' : '' }}>Other</option>
                                                </select>
                                            </div>
                                            <div class="form-group other-degree-input" id="other-degree-{{ $index }}" style="display: {{ $isOther && $degree ? 'block' : 'none' }};">
                                                <input type="text" class="form-control" placeholder="Enter degree name" name="education[{{ $index }}][degree_other]" value="{{ $isOther ? $degree : '' }}">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
