...
class StaffController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('education')) {
            foreach ($request->education as $education) {
                ...
                if (isset($education['degree']) && $education['degree'] === 'Other' && !empty($education['degree_other'])) {
                    $education['degree'] = $education['degree_other'];
                }
                unset($education['degree_other']); // Remove degree_other from data
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle education, media, and research updates
        // Delete existing education, media, and research records
        $staff->education()->delete();
        $staff->media()->delete();
        $staff->research()->delete();
        ...
        if ($request->has('education')) {
            foreach ($request->education as $education) {
                ...
                if (!empty($education['degree']) || !empty($education['major']) || !empty($education['institution'])) {
                    $staff->education()->create($education);
                }
            }
        }
        ...
        if ($request->has('research')) {
            foreach ($request->research as $research) {
                if (!empty($research['research_field']) || !empty($research['description'])) {
                    $staff->research()->create($research);
                }
            }
        }
